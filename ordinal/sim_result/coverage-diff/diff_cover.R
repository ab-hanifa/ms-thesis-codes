...
dat_ord_diff_fn <- function(file_lst, path = "ordinal/sim_result/coverage-diff/") {
...
file_lst <- file_path_sans_ext(
  list.files(path = "ordinal/sim_result/coverage-diff/", 
             pattern = "\\.Rdata$", 
             full.names = FALSE)
)

# Apply the function
diff_cover_dat <- dat_ord_diff_fn(file_lst)
...
ordi_rb_dat_diff <- map(diff_cover_dat, rb_fn) |> 
  list_rbind() |> 
...
ordi_rb_dat_diff |> view()
save(ordi_rb_dat_diff, file = "ordi_rb_dat_diff.Rdata")
