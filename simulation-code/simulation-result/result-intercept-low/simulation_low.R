...
cat_glmadap_all_low <- parallel::mclapply(1:24, function(p) {
  lapply(data_low, function(x) {
    estimation_fn_int(datx = x[[p]], sigma2 = c(0.55, 0.75))
  })
},
mc.cores = 4)

save(cat_glmadap_all_low, 
     file = "cat_glmadap_all_low.Rdata")
